...
class Effect:
    ...
    def apply_scan_effect(image):
        # 蜷曲
        image = Effect.apply_scan_curve_effect(image)
        # 扫描线
        image = Effect.apply_scan_line_effect(image)
        # 噪点
        image = Effect.apply_scan_noise_effect(image)
    ...
    @staticmethod
    def apply_scan_line_effect(image, probability=0.01):
        # 创建新的图像，与原图像大小相同
    ...
    @staticmethod
    def apply_scan_noise_effect(image, probability=0.001):
        # 创建新的图像，与原图像大小相同
        new_image = Image.new("RGB", image.size)
        draw = ImageDraw.Draw(new_image)
        width, height = image.size
    ...
    @staticmethod
    def apply_scan_curve_effect(image):
        width = image.width
        height = image.height
